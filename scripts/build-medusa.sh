...
# Mirror the repo install contract used by CI. Lifecycle scripts stay off;
# the std/plugin build step below produces the artifacts installs would have.
pnpm install --frozen-lockfile --prefer-offline --ignore-scripts --strict-peer-dependencies --filter=medusa-be...
step_end "Install"
...
pnpm install --offline --frozen-lockfile --ignore-scripts --strict-peer-dependencies --filter=medusa-be...
