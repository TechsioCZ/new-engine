...
run_tests() {
  ...
  vitest_args=(run --config vitest.config.mts --no-file-parallelism)
...
Environment:
  ...
  MEDUSA_E2E_TEST_TARGET      Optional space-separated Vitest targets or file filters (default: all Docker-backed HTTP e2e specs from vitest.config.mts)
USAGE
}
