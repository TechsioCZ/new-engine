...
### Strategy B — Code owns fluid, atoms alias primitives _(current)_
...
1. Is the value identical to a sibling reference-layer token (`--color-X-{bg|fg|border}-Y` ↔ `--color-X-Y`)? → emit `var(--ref)` (CLAUDE.md two-layer convention)
2. Does the value (with L = D) match a known primitive of the same property kind? → emit `var(--primitive)` (this strategy)
3. Does L ≠ D in Figma? → emit `light-dark(L, D)`
4. Default → emit the literal value
