...
const toEffectBaseUrl = (apiBaseUrl: string) =>
  apiBaseUrl.startsWith("/")
    ? `${internalRelativeOrigin}${apiBaseUrl}`
    : apiBaseUrl

const toPublicUrl = (url: string) =>
  url.startsWith(internalRelativeOrigin)
    ? url.slice(internalRelativeOrigin.length) || "/"
    : url
...
const requestInitFromWebRequest = (
  request: Request,
  signal: AbortSignal
): RequestInit => {
...
const isSmartSuggestErrorPayload = (
  value: unknown
): value is SmartSuggestError =>
  typeof value === "object" &&
  value !== null &&
  typeof Reflect.get(value, "code") === "string" &&
  typeof Reflect.get(value, "message") === "string"

const extractClientErrorPayload = (
  value: unknown
): SmartSuggestClientErrorPayload => {
...
const toClientError = (error: unknown) => {
  ...
  if (sharedStatus !== undefined) {
    return new SmartSuggestClientError(
      sharedStatus,
      extractClientErrorPayload(error)
    )
  }
...
const createApiClientEffect = (
  ...
  HttpApiClient.makeWith(SmartSuggestHttpApi, {
    ...
    httpClient: createHttpClient(fetchImpl, requestSignal),
  })
...
const toSuggestQuery = (request: SmartSuggestRequest) => ({
  ...
  ...(request.tenant?.cartId === undefined
    ? {}
    : { cartId: request.tenant.cartId }),
  ...(request.tenant?.sessionId === undefined
    ? {}
...
export const createSmartSuggestEffectClient = (
  ...
  const runWithRequestEffect = <TResponse>(
    ...
    sync(() => createRequestSignal(requestOptions, timeoutMs)).pipe(
      flatMap((requestSignal) =>
        createApiClientEffect(apiBaseUrl, fetchImpl, requestSignal.signal).pipe(
  ...
  return {
    ...
    validatePhone: (request, requestOptions = {}) =>
      runWithRequestEffect(requestOptions, (client) =>
        client.validatePhone({ payload: request })
      ),
    validatePostal: (request, requestOptions = {}) =>
      runWithRequestEffect(requestOptions, (client) =>
        client.validatePostal({ payload: request })
      ),
  }
}
