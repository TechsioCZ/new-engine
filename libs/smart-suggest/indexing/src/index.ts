...
const STREET_NUMBER_PATTERN =
  /(?<![\p{L}\p{N}/])(\d{1,5}[a-zA-Z]?)(?![\p{L}\p{N}/])/gu
const CITY_DISTRICT_NUMBER_PATTERN =
  /\b(?:praha|bratislava|ostrava|brno|kosice|plzen)$/
...
export const createPrefixTokens = (
  ...
  const minLength = Math.max(
    1,
    Math.trunc(options.minLength ?? DEFAULT_MIN_PREFIX_LENGTH)
  )
  const maxLength = Math.max(
    minLength,
    Math.trunc(options.maxLength ?? DEFAULT_MAX_PREFIX_LENGTH)
  )
  const seen = new Set<string>()
...
const formatCzSkPostalCode = (digits: string) =>
  `${digits.slice(0, 3)} ${digits.slice(3)}`

export const extractPostalCodeCandidates = (
  value: string
): PostalCodeCandidate[] => {
...
const isLikelyCityDistrictNumber = (value: string, start: number) => {
  const before = normalizeSearchText(
    value.slice(Math.max(0, start - 16), start)
  )

  return CITY_DISTRICT_NUMBER_PATTERN.test(before)
}

const normalizeAddressNumber = (value: string) =>
  stripDiacritics(value).toLocaleLowerCase("cs-CZ")
...
export const extractHouseNumberCandidates = (
  value: string
  ...
  const pushCandidate = (
    ...
    const key = createHouseCandidateKey(
      houseNumber,
      orientationNumber,
      candidate.start
    )
    ...
    seen.add(key)
    const displayValue =
      candidate.reason === "explicit-orientation" ||
...
const joinNonEmpty = (
  ...
  values
    .map((value) =>
      value === undefined ? undefined : normalizeDisplayText(String(value))
    )
...
export const createAddressLabels = (
  ...
  const regionCountryLine = joinNonEmpty(
    [parts.region, parts.countryCode?.toUpperCase()],
    ", "
  )
  const displayLabel =
...
const addressPartsFromCandidate = (
  candidate: AddressRankingCandidate
): AddressIndexParts => candidate.parts ?? candidate.address ?? {}
...
const labelTextFromCandidate = (candidate: AddressRankingCandidate) => {
  ...
  if (
    candidate.displayLabel !== undefined &&
    candidate.displayLabel.length > 0
  ) {
...
const normalizePostalCodeValue = (value: string | undefined) =>
...
const collectCandidatePostalCodes = (candidate: AddressRankingCandidate) => {
  ...
  for (const postalCode of extractPostalCodeCandidates(
    labelTextFromCandidate(candidate)
  )) {
...
const collectCandidateHouseNumbers = (candidate: AddressRankingCandidate) => {
  const values = new Set<string>()
  const parts = addressPartsFromCandidate(candidate)

  if (parts.houseNumber !== undefined) {
    values.add(normalizeAddressNumber(parts.houseNumber))
  }

  if (parts.orientationNumber !== undefined) {
    values.add(normalizeAddressNumber(parts.orientationNumber))
  }
  ...
  if (
    ...
    values.add(
      `${normalizeAddressNumber(parts.houseNumber)}/${normalizeAddressNumber(
        parts.orientationNumber
      )}`
    )
  }

  for (const houseNumber of extractHouseNumberCandidates(
    labelTextFromCandidate(candidate)
  )) {
    values.add(houseNumber.houseNumber)

    if (houseNumber.orientationNumber !== undefined) {
      values.add(houseNumber.orientationNumber)
      values.add(`${houseNumber.houseNumber}/${houseNumber.orientationNumber}`)
    }
  }
...
const applyLabelScore = (
  ...
  if (
    ...
    normalizedCandidateLabel === normalizedQuery
  ) {
    state.score += 100
  ...
  if (
    normalizedQuery.length > 0 &&
    normalizedCandidateLabel.startsWith(normalizedQuery)
  ) {
    state.score += 20
...
const applyTokenScore = (
  ...
  if (
    ...
    exactTokenMatches.length === queryTokens.length
  ) {
    state.score += 30
...
const applyPostalScore = (
  state: ScoreState,
  query: string,
  candidate: AddressRankingCandidate
) => {
  const candidatePostalCodes = collectCandidatePostalCodes(candidate)
  const matchedPostalCodes = extractPostalCodeCandidates(query).filter(
    (postalCode) => candidatePostalCodes.has(postalCode.value)
  )
...
const applyHouseNumberScore = (
  ...
  const candidateHouseNumbers = collectCandidateHouseNumbers(candidate)
  const matchedHouseNumbers = extractHouseNumberCandidates(query).filter(
    (houseNumber) =>
      candidateHouseNumbers.has(houseNumber.houseNumber) ||
      (houseNumber.orientationNumber !== undefined &&
        candidateHouseNumbers.has(
          `${houseNumber.houseNumber}/${houseNumber.orientationNumber}`
        ))
  )
...
export const scoreAddressCandidate = <
  TCandidate extends AddressRankingCandidate,
>(
  query: string,
  candidate: TCandidate
...
export const rankAddressCandidates = <
  TCandidate extends AddressRankingCandidate,
>(
  query: string,
