...
const defaultFetch: SmartSuggestVanillaFetch = (input, init) =>
  fetch(input, init)
...
const resolveField = (
  ...
  field: SmartSuggestVanillaField
) => {
...
const getControlValue = (control: TextControl | undefined) =>
  control?.value.trim() ?? ""
...
const reportError = (
  onError: SmartSuggestVanillaConfig["onError"],
  error: unknown
) => {
  onError?.(error)
}
...
const requestJson = async <TResponse>(
  ...
  const timeout = setTimeout(() => {
    controller.abort(
      new DOMException("Smart Suggest request timed out.", "TimeoutError")
    )
  }, timeoutMs)
  ...
  const validatePhone = async () => {
    ...
    try {
      const result = await postJson<SmartSuggestVanillaValidationResult>({
        apiBaseUrl,
        body: { defaultCountry: readCountryCode(), rawInput },
        fetchImpl,
