...
const TOP_LEVEL_VALIDATION_IMPORT_PATTERN =
  /@techsio\/smart-suggest-validation["']/u
const LIBPHONENUMBER_IMPORT_PATTERN = /libphonenumber-js/u
...
describe("attachSmartSuggest", () => {
  ...
  it("uses an anchored popover combobox and supports keyboard selection", async () => {
    ...
    expect(address.getAttribute("aria-controls")).toBe(
      (list as HTMLUListElement).id
    )
    ...
    expect(
      (list as HTMLUListElement).style.getPropertyValue("position-anchor")
    ).toBe(address.style.getPropertyValue("anchor-name"))
