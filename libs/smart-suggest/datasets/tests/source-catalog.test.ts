...
describe('smart suggest source catalog', () => {
  ...
  it('exports the required source policy entries', () => {
    expect(SMART_SUGGEST_SOURCE_IDS).toEqual(expect.arrayContaining(requiredSourceIds));
    expect(SMART_SUGGEST_SOURCE_POLICIES).toHaveLength(SMART_SUGGEST_SOURCE_IDS.length);
  ...
  it('allows RUIAN CZ permanent owned imports with attribution and refresh metadata', () => {
    ...
    expect(smartSuggestSourceAllowsWrite('ruian-cz', 'permanent-cache')).toBe(true);
    expect(smartSuggestSourceAllowsWrite('ruian-cz', 'durable-retention')).toBe(true);
    ...
    expect(() => assertSmartSuggestSourceAllowsPermanentImport('ruian-cz')).not.toThrow();
  });
  ...
  it('blocks unsafe permanent source writes for live, pending, and blanket sources', () => {
    ...
    for (const sourceId of unsafePermanentSourceIds) {
      expect(smartSuggestSourceAllowsWrite(sourceId, 'permanent-cache')).toBe(false);
      expect(smartSuggestSourceAllowsWrite(sourceId, 'durable-retention')).toBe(false);
      ...
      expect(() => assertSmartSuggestSourceAllowsPermanentImport(sourceId)).toThrow(
        SmartSuggestPermanentImportPolicyError,
      );
      expect(() => assertSmartSuggestSourceAllowsPermanentImport(sourceId)).toThrow(
        'permanent Smart Suggest source writes',
      );
    }
  });
  ...
  it('allows deployment-approved live providers to save provider results permanently', () => {
    ...
    for (const sourceId of deploymentApprovedProviderSourceIds) {
      ...
      expect(smartSuggestSourceAllowsWrite(sourceId, 'permanent-cache')).toBe(true);
      expect(smartSuggestSourceAllowsWrite(sourceId, 'durable-retention')).toBe(true);
      expect(smartSuggestSourceAllowsWrite(sourceId, 'bulk-import')).toBe(false);
      expect(smartSuggestSourceAllowsPermanentImport(sourceId)).toBe(false);
      expect(() => assertSmartSuggestSourceAllowsPermanentImport(sourceId)).toThrow(
        SmartSuggestPermanentImportPolicyError,
      ...
      expect(() => assertSmartSuggestSourceAllowsPermanentImport(sourceId)).toThrow(
        'bulk-import/backfill permission',
      );
    }
  });
  ...
  it('keeps live provider cache policy explicit', () => {
    ...
    expect(smartSuggestSourceAllowsTtlCache('radar-autocomplete', 30)).toBe(true);
    expect(smartSuggestSourceAllowsTtlCache('radar-autocomplete', 31)).toBe(false);
  ...
  it('throws schema-backed tagged errors for unknown source policies', () => {
    expect(() => requireSmartSuggestSourcePolicy('missing-source')).toThrow(
      SmartSuggestSourcePolicyNotFoundError,
    );

    try {
      requireSmartSuggestSourcePolicy('missing-source');
      throw new Error('Expected missing source policy to throw.');
    } catch (error) {
      expect(error).toMatchObject({
        _tag: 'SmartSuggestSourcePolicyNotFoundError',
