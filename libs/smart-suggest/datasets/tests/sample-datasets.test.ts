...
import {
  ...
  SAMPLE_ADDRESS_FIXTURE_USAGE,
  searchSampleAddressFixtureProofs,
  seedSampleAddressFixtureDatasetsEffect,
} from "../src/sample-fixtures";

const runAddressDatasetImport = (options: Parameters<typeof runAddressDatasetImportEffect>[0]) =>
  runAddressDatasetImportEffect(options);

const seedSampleAddressDatasets = (
  repositories: Parameters<typeof seedSampleAddressFixtureDatasetsEffect>[0],
) => seedSampleAddressFixtureDatasetsEffect(repositories);

describe("sample smart suggest datasets", () => {
  it("marks sample helpers as fixture-only, not runtime BFF seed behavior", () => {
    expect(SAMPLE_ADDRESS_FIXTURE_USAGE).toMatchObject({
      kind: "sample-fixture",
      productionRuntime: false,
    });
    expect(SAMPLE_ADDRESS_FIXTURE_USAGE.allowedUses).toEqual(
      expect.arrayContaining(["dataset tests", "import proof tests"]),
    );
    expect(SAMPLE_ADDRESS_FIXTURE_USAGE.forbiddenUses).toEqual(
      expect.arrayContaining(["runtime BFF startup", "production seed"]),
    );
  });
  ...
  it.effect("seeds source registry and normalized records through repository APIs", () =>
    gen(function* sampleSeedProgram() {
      ...
      expect(result.records.filter((record) => record.parts.street === "K Louži")).toHaveLength(4);

      const source = yield* repositories.dataSources.getDataSource(RUIAN_CZ_SAMPLE_SOURCE.id);
  ...
  it("searches CZ and SK sample records with diacritics-insensitive ranking", () => {
    expect(
      searchSampleAddressFixtureProofs("vaclavske 832", {
        countryCode: "CZ",
        limit: 1,
      })[0],
    ).toMatchObject({
    ...
    expect(
      searchSampleAddressFixtureProofs("K Louži 1258/12", {
        countryCode: "CZ",
        limit: 1,
      })[0],
    ).toMatchObject({
    ...
    expect(
      searchSampleAddressFixtureProofs("K Lou", {
        countryCode: "CZ",
        limit: 5,
    ...
    expect(
      searchSampleAddressFixtureProofs("zizkova zilina", {
        countryCode: "SK",
        limit: 1,
      })[0],
    ).toMatchObject({
    ...
    expect(
      searchSampleAddressFixtureProofs("mission san francisco", {
        countryCode: "US",
        limit: 1,
      })[0],
    ).toMatchObject({
  ...
  it.effect("rejects sample fixture sources in the production import runner", () =>
    gen(function* sampleFixtureImportGuardProgram() {
      const repositories = createInMemorySmartSuggestRepositories();
      const importExit = yield* effectExit(
        runAddressDatasetImport({
          ...
          rows: [
            {
              id: "cz-ruian-sample-fixture-runtime-guard",
              parts: {
                city: "Praha",
                countryCode: "CZ",
                houseNumber: "1",
          ...
          runId: "import-sample-fixture-runtime-guard",
          source: {
            ...RUIAN_CZ_SAMPLE_SOURCE,
            shardCountryCode: "CZ",
          },
      ...
      const storedSource = yield* repositories.dataSources.getDataSource(RUIAN_CZ_SAMPLE_SOURCE.id);
      const importRun = yield* repositories.importRuns.getImportRun(
        "import-sample-fixture-runtime-guard",
      );
      ...
      expect(squash(importExit.cause)).toMatchObject({
        message: expect.stringContaining('Unknown Smart Suggest source policy "ruian-cz-sample"'),
      });
  ...
  it.effect("runs RUIAN chunked imports with bad-row accounting and restartable upserts", () =>
    gen(function* ruianChunkedImportProgram() {
      ...
      const prikopeRecord =
        yield* repositories.addressRecords.getAddressRecord("cz-ruian-na-prikope-10");
  ...
  it.effect("marks import runs failed when repository writes fail", () =>
    gen(function* failedImportRunProgram() {
      ...
      const failingRepositories = {
        ...
        addressRecords: {
          ...
          upsertAddressRecords: () =>
            fail(new SmartSuggestStorageError("storage-unavailable", "D1 write failed")),
        },
      };
      ...
      const failedRun = yield* repositories.importRuns.getImportRun("import-ruian-cz-failed");
