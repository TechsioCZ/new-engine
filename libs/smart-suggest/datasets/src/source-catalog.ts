...
export type SmartSuggestCatalogSourceKind = Exclude<
  SuggestionSourceKind,
  "cache"
>
...
export const SMART_SUGGEST_SOURCE_CATALOG = {
  "ruian-cz": {
    attribution: {
      ...
      notes: [
        "Show CUZK/RUIAN attribution and note Smart Suggest modifications.",
      ],
      required: true,
    ...
    bulkImport: {
      allowed: true,
      notes: [
        "Bulk import is allowed from Atom-discovered RUIAN exchange snapshots.",
      ],
    },
    cachePolicy: {
      kind: "permanent",
    ...
    notes: [
      "Discover the current ZIP through Atom; do not hardcode dated ZIP URLs.",
    ],
    refresh: {
      cadence: "monthly",
      discovery: "atom",
  ...
  "register-adries-sk": {
    attribution: {
      ...
      notes: [
        "Official Slovakia address register attribution and license are not confirmed.",
      ],
      required: true,
    ...
    bulkImport: {
      allowed: false,
      notes: [
        "Promising owned dataset candidate, but production import is blocked for now.",
      ],
      reason:
        "License and attribution terms must be confirmed before production import.",
    },
    cachePolicy: {
      kind: "pending-license",
    ...
    durableRetention: {
      allowed: false,
      notes: [
        "Do not retain production records until the license is confirmed.",
      ],
    ...
    notes: [
      "Keep this source pending; do not enable permanent production import yet.",
    ],
    refresh: {
      cadence: "pending-license",
      discovery: "manual",
      notes: [
        "Refresh cadence must be decided with the confirmed license terms.",
      ],
      required: true,
    },
    sourceKind: "owned-dataset",
  ...
  openaddresses: {
    attribution: {
      ...
      modificationNoticeRequired: false,
      notes: [
        "OpenAddresses attribution and license must be resolved per source config.",
      ],
      required: true,
    ...
    bulkImport: {
      allowed: false,
      notes: [
        "A per-source catalog entry must approve import before production use.",
      ],
      reason:
        "OpenAddresses is an aggregator and has no blanket permanent import policy.",
    },
    cachePolicy: {
      kind: "per-source",
      notes: [
        "Never treat OpenAddresses as one blanket permanent cache source.",
      ],
    },
    countryCoverage: {
      kind: "source-dependent",
    ...
    durableRetention: {
      allowed: false,
      notes: [
        "Blanket durable retention is blocked until a child source policy permits it.",
      ],
      reason:
        "Each OpenAddresses source has its own license and attribution requirements.",
    },
    id: "openaddresses",
    name: "OpenAddresses blanket source",
    notes: [
      "Represent approved OpenAddresses imports as per-source child policies.",
    ],
    refresh: {
      cadence: "per-source",
      discovery: "per-source-policy",
      notes: [
        "Refresh requirements must come from each approved OpenAddresses source.",
      ],
      required: true,
    },
    sourceKind: "owned-dataset",
  ...
  "radar-autocomplete": {
    ...
    bulkImport: {
      allowed: false,
      notes: [
        "Do not import Radar suggestions into the address DB or offline index.",
      ],
    ...
    notes: [
      "Allowed only as live fallback/enrichment with a max 30 day TTL cache.",
    ],
    refresh: {
      cadence: "none",
      discovery: "none",
      notes: [
        "Cache entries must expire within the approved TTL; no dataset refresh applies.",
      ],
      required: false,
    },
    sourceKind: "live-provider",
  ...
  "nominatim-managed": {
    ...
    bulkImport: {
      ...
      reason:
        "No separate Nominatim bulk-import/backfill permission is recorded.",
    },
    cachePolicy: {
      kind: "permanent",
      notes: [
  ...
  "nominatim-public": {
    ...
    bulkImport: {
      allowed: false,
      notes: [
        "Do not use the public Nominatim service for bulk import or index building.",
      ],
      reason:
        "Public Nominatim is not allowed for autocomplete or production fallback.",
    },
    cachePolicy: {
      kind: "none",
    ...
    countryCoverage: {
      kind: "global",
      notes: [
        "Public service coverage is irrelevant because the source is blocked for this use.",
      ],
    },
    durableRetention: {
      allowed: false,
      ...
      reason:
        "Public Nominatim is not allowed for this production autocomplete path.",
    },
    id: "nominatim-public",
    name: "Public Nominatim service",
    notes: [
      "Exclude from autocomplete and production provider fallback priority.",
    ],
    refresh: {
      cadence: "none",
      discovery: "none",
  ...
  "ruian-geocode": {
    attribution: {
      ...
      modificationNoticeRequired: true,
      notes: [
        "Verify endpoint-specific terms before retaining live geocoder results.",
      ],
      required: true,
      url: "https://ruian.cuzk.cz/",
    },
    bulkImport: {
      allowed: false,
      notes: [
        "Prefer the owned RUIAN bulk import path instead of live geocoder retention.",
      ],
    ...
    durableRetention: {
      allowed: false,
      notes: [
        "Do not persist live geocoder results until endpoint terms are verified.",
      ],
    ...
    refresh: {
      ...
      notes: [
        "Endpoint terms must be verified before changing retention policy.",
      ],
      required: true,
    },
    sourceKind: "live-provider",
...
export type SmartSuggestSourceWriteTarget =
  | "bulk-import"
  | "durable-retention"
  | "permanent-cache"

export type SmartSuggestSourcePolicyReference =
  | SmartSuggestSourcePolicy
  | string
...
export const SMART_SUGGEST_SOURCE_POLICIES: readonly SmartSuggestSourcePolicy[] =
  Object.values(SMART_SUGGEST_SOURCE_CATALOG)

export const isSmartSuggestSourceId = (
  sourceId: string
): sourceId is SmartSuggestSourceId => sourceId in SMART_SUGGEST_SOURCE_CATALOG
...
export const resolveSmartSuggestSourceId = (
  sourceId: string
): SmartSuggestSourceId | undefined => {
...
export const requireSmartSuggestSourcePolicy = (
  sourceId: string
): SmartSuggestSourcePolicy => {
  const policy = getSmartSuggestSourcePolicy(sourceId)

  if (policy === undefined) {
    throw new Error(`Unknown Smart Suggest source policy "${sourceId}".`)
  }
...
export const smartSuggestSourceMaxTtlDays = (
  ...
  if (
    policy?.cachePolicy.kind === "ttl" ||
    policy?.cachePolicy.kind === "conditional-ttl"
  ) {
...
export const smartSuggestSourceAllowsTtlCache = (
  ...
  if (policy.cachePolicy.kind === "conditional-ttl") {
    return (
      context.deploymentAllowsTtl === true &&
      ttlDays <= policy.cachePolicy.maxTtlDays
    )
  }
...
export const assertSmartSuggestSourceAllowsPermanentImport = (
  ...
  if (!smartSuggestSourceAllowsPermanentImport(policy)) {
    throw new Error(
      `Source "${sourceId}" is not allowed for permanent Smart Suggest source writes: ${explainPermanentSourceWriteBlock(
        policy
      )}.`
    )
  }
