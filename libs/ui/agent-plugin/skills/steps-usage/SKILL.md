...
Use Steps for wizard/progress workflows. Use Tabs for peer panels, and Pagination for page navigation.
...
<Steps count={3} linear>
  ...
  <Steps.Panels>
    <Steps.Content index={0}>Cart content</Steps.Content>
  </Steps.Panels>
  <Steps.Navigation>
...
Use `PrevTrigger` and `NextTrigger` so disabled/completed state comes from the machine.
