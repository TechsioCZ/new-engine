...
2. <b>Bring the full local stack to steady state</b>
    ...
    * Default `mise run dev` does not require operator credentials; operator flow is explicit opt-in and requires:
    ...
    First-time upstream ZaneOps setup assumptions for local deploy testing:
      ...
      * service names in that Zane project must match `apps/new-engine-ctl/config/stack-manifest.yaml` currently: `medusa-db`, `medusa-valkey`, `medusa-minio`, `medusa-meilisearch`, `medusa-be`, `payload`, `herbatika`; `n1` remains an explicitly selectable reference storefront and is excluded from automatic CI scope
