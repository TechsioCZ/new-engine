...
export const listSearchProfiles = async (): Promise<SearchProfile[]> => {
	const response = await medusaClient.client.fetch<{ profiles: SearchProfile[] }>('/admin/search-profiles')

	return response.profiles
}

export const listSalesChannels = async (): Promise<SalesChannelOption[]> => {
	const response = await medusaClient.client.fetch<{ sales_channels: SalesChannelOption[] }>('/admin/sales-channels', { query: { fields: 'id,name', limit: 100 } })

	return response.sales_channels
}

export const createSearchProfile = async (input: SearchProfileInput): Promise<SearchProfile> => {
	const response = await medusaClient.client.fetch<{ profile: SearchProfile }>('/admin/search-profiles', { method: 'POST', body: input })

	return response.profile
}

export const updateSearchProfile = async (id: string, input: SearchProfileInput): Promise<SearchProfile> => {
	const response = await medusaClient.client.fetch<{ profile: SearchProfile }>('/admin/search-profiles/' + id, { method: 'POST', body: input })

	return response.profile
}

export const deleteSearchProfile = async (id: string): Promise<void> => {
	await medusaClient.client.fetch('/admin/search-profiles/' + id, { method: 'DELETE' })
}

export const synchronizeSearchProfiles = async (mode: SearchSyncMode, id?: string) => medusaClient.client.fetch<{ result: { deleted: number; indexed: number; mode: SearchSyncMode; profiles: number } }>(id ? '/admin/search-profiles/' + id + '/sync' : '/admin/search-profiles/sync', { method: 'POST', body: { mode } })

export const testSearchProfile = async (id: string, input: SearchTestInput): Promise<SearchTestResult> => medusaClient.client.fetch<SearchTestResult>('/admin/search-profiles/' + id + '/test', { method: 'POST', body: input })

export const getVectorStatus = async (): Promise<VectorStatus> => medusaClient.client.fetch('/admin/meilisearch/vector-status')

export const getMeilisearchStatus = async (): Promise<MeilisearchStatus> => medusaClient.client.fetch('/admin/search-profiles/status')
...
export const toSearchProfileInput = (profile?: SearchProfile): SearchProfileInput => {
	if (!profile) {
