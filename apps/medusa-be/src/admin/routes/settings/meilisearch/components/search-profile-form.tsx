import { Button, Checkbox, FocusModal, Heading, Input, Label, Select, Switch, Text } from '@medusajs/ui'
import type { Dispatch, FormEvent, SetStateAction } from 'react'
import { useEffect, useMemo, useState } from 'react'
import { type SalesChannelOption, type SearchIndexType, type SearchProfile, type SearchProfileInput, toSearchProfileInput } from '../../../../lib/search-profiles'

const INDEX_TYPES: SearchIndexType[] = ['product', 'category', 'brand', 'content']

type AutocompleteLimitField = 'autocomplete_product_limit' | 'autocomplete_category_limit' | 'autocomplete_brand_limit' | 'autocomplete_content_limit'

const AUTOCOMPLETE_LIMIT_FIELD_BY_TYPE: Record<SearchIndexType, AutocompleteLimitField> = {
...
const NumberField = ({ description, label, max, min = 1, onChange, value }: {
	description: string
	label: string
	max: number
	min?: number
...
const ToggleField = ({ checked, description, label, onCheckedChange }: {
	checked: boolean
	description: string
	label: string
...
const normalizeProfileKeySegment = (value: string) => value.trim().toLowerCase().normalize('NFKD').replaceAll(/\p{Diacritic}/gu, '').replaceAll(/[^a-z0-9_-]+/g, '-').replaceAll(/-+/g, '-').replaceAll(/^-|-$/g, '')
const generatedProfileKey = (form: SearchProfileInput) => [form.shop, form.domain, form.locale].map(normalizeProfileKeySegment).filter(Boolean).join('-')

const SearchProfileFields = ({ form, salesChannels, setForm }: {
	form: SearchProfileInput
	salesChannels: SalesChannelOption[]
	setForm: Dispatch<SetStateAction<SearchProfileInput>>
}) => {
	const effectiveScore = form.minimum_ranking_score ?? (form.strict ? 0.98 : 0.55)

	const toggleSalesChannel = (id: string, checked: boolean) => {
		setForm((current) => ({
			...current,
			sales_channel_ids: checked ? [...new Set([...current.sales_channel_ids, id])] : current.sales_channel_ids.filter((entry) => entry !== id)
		}))
	}
							...
							salesChannels.map((channel) => (
								...
								>
									<Checkbox
										...
										onCheckedChange = {
											(checked) => toggleSalesChannel(channel.id, checked === true)
										}
									/>
			...
			<section className = 'flex flex-col gap-4'>
				...
				<div className = 'grid grid-cols-1 gap-3 md:grid-cols-2'>
					<ToggleField
						...
						onCheckedChange = {
							(strict) => setForm((current) => {
								const scoreFollowedPreviousDefault = current.minimum_ranking_score === (current.strict ? 0.98 : 0.55)

								return { ...current, strict: strict, minimum_ranking_score: scoreFollowedPreviousDefault ? null : current.minimum_ranking_score }
							})
						}
					/>
					...
					<ToggleField
						...
						onCheckedChange = {
							(separateVariantResults) => setForm((current) => ({
								...current,
				...
				<div className = 'grid grid-cols-1 gap-4 md:grid-cols-2'>
					<div className = 'flex flex-col gap-2'>
						<Label
							htmlFor = 'search-profile-availability'
						>
							Availability
						</Label>
						...
						>
							<Select.Trigger
								id = 'search-profile-availability'
							>
								<Select.Value />
							</Select.Trigger>
					...
					<div className = 'flex flex-col gap-2'>
						<Label
							htmlFor = 'search-profile-ranking-score'
						>
						...
						<Input
							...
							onChange = {
								(event) => setForm((current) => ({
									...current,
									minimum_ranking_score: event.target.value === '' ? null : Number(event.target.value)
								}))
							}
						/>
						...
						<Text
							...
							size = 'xsmall'
						>
							Effective score: { effectiveScore }. Leave empty to follow the strict/loose default.
						</Text>
					</div>
				</div>
			</section>
...
export const SearchProfileFormModal = ({ onOpenChange, onSubmit, open, profile, salesChannels, submitting }: {
	...
	const [form, setForm] = useState<SearchProfileInput>(() => toSearchProfileInput(profile))
	...
	const valid = useMemo(
		() =>
			[generatedProfileKey(form), form.shop, form.domain, form.locale].every((value) => value.trim().length > 0) &&
			[
			...
			(form.minimum_ranking_score === null || (form.minimum_ranking_score >= 0 && form.minimum_ranking_score <= 1)),
