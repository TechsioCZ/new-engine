...
export const SearchTestPanel = ({
  ...
  const mutationOptions = {
    mutationFn: () => testSearchProfile(profileId, { query, type, limit: 10 }),

    onError: (error: unknown) => {
      toast.error(
        error instanceof Error ? error.message : "Search testing failed."
      )
    },
    ...
    <div className="flex flex-col gap-4 px-6 py-5">
      <div>
        ...
        <Text className="text-ui-fg-subtle" size="small">
          Query the exact index and relevance policy selected for a storefront
          profile. An empty product query tests popular-product ordering.
        </Text>
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_180px_2fr_auto] md:items-end">
        <div className="flex flex-col gap-2">
          <Label htmlFor="search-test-profile">Profile</Label>

          <Select onValueChange={setProfileId} value={profileId}>
            <Select.Trigger id="search-test-profile">
        ...
        <div className="flex flex-col gap-2">
          <Label htmlFor="search-test-type">Index type</Label>

          <Select
            onValueChange={(value) => setType(value as SearchIndexType)}
            value={type}
          >
            <Select.Trigger id="search-test-type">
              <Select.Value />
            </Select.Trigger>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-ui-border-base border-b px-4 py-3">
            ...
            <Text className="text-ui-fg-subtle" size="xsmall">
              ...
              {result.minimum_ranking_score !== null
                ? ` · min score ${result.minimum_ranking_score}`
                : ""}
            </Text>
          </div>
              ...
              {result.hits.length ? (
                result.hits.map((hit, index) => (
                  <Table.Row key={hitId(hit, index)}>
                    <Table.Cell>{hitLabel(hit)}</Table.Cell>
                ...
                <Table.Row>
                  <Table.Cell>No accepted results.</Table.Cell>
