...
const isFulfillmentRecord = (value: unknown): value is { id?: string | null } => isRecord(value) && isOptionalString(value.id)

const isOrderFulfillmentRecord = (value: unknown): value is OrderFulfillmentRecord => {
	if (!isRecord(value)) {
		return false
	}

	const fulfillments: unknown = value.fulfillments
	return fulfillments === undefined || fulfillments === null || (Array.isArray(fulfillments) && fulfillments.every(isFulfillmentRecord))
}
...
export function buildGLSFulfillmentOperationIdentity(input: FulfillmentOperation): FulfillmentOperationIdentity {
	...
	const accessPointId = typeof input.attributes.addressId === 'string' ? input.attributes.addressId.trim() || null : null
	const currency = typeof input.attributes.currency === 'string' ? input.attributes.currency.trim().toUpperCase() || null : null
	const payload = JSON.stringify({ environment: input.environment, orderId: input.orderId, items: normalizedItems, accessPointId, cashOnDelivery: input.attributes.cod !== undefined, currency })
