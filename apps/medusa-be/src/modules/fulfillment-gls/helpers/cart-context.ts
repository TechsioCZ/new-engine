...
const isCountryRecord = (value: unknown): value is { iso_2?: string | null } =>
  isRecord(value) && isOptionalString(value.iso_2)

const isCountryRecordArray = (
  value: unknown
): value is Array<{ iso_2?: string | null }> =>
  Array.isArray(value) && value.every(isCountryRecord)

const isShippingAddressRecord = (
  value: unknown
): value is { country_code?: string | null } =>
  isRecord(value) && isOptionalString(value.country_code)

const isCartContextRecord = (value: unknown): value is CartContextRecord => {
  if (
    !isRecord(value) ||
    typeof value.id !== "string" ||
    !isOptionalString(value.sales_channel_id)
  ) {
    return false
  }
  ...
  if (
    ...
    !isShippingAddressRecord(value.shipping_address)
  ) {
    return false
  }
  ...
  if (value.region !== undefined && value.region !== null) {
    ...
    if (
      countries !== undefined &&
      countries !== null &&
      !isCountryRecordArray(countries)
    ) {
      return false
    }
  }
...
const isGLSStorefrontCountryCode = (
  value: string
): value is GLSStorefrontCountryCode => STOREFRONT_COUNTRY_CODE_SET.has(value)

export async function resolveGLSCartContext(
  query: QueryService,
  cartId: string
): Promise<GLSCartContext> {
  ...
  if (!salesChannelId) {
    throw new MedusaError(
      ...
      "Cart has no Sales Channel"
    )
  }

  const regionCountryCodes = (cart.region?.countries ?? [])
    .map((country) => country.iso_2?.trim().toUpperCase())
    .filter((candidateCountryCode): candidateCountryCode is string =>
      Boolean(candidateCountryCode)
    )
  const addressCountryCode = cart.shipping_address?.country_code
    ?.trim()
    .toUpperCase()
  const countryCode =
    addressCountryCode ??
    (regionCountryCodes.length === 1 ? regionCountryCodes[0] : undefined)

  if (!countryCode) {
    throw new MedusaError(
      MedusaError.Types.NOT_ALLOWED,
      "GLS is unavailable for this cart country"
    )
  }
  ...
  if (addressCountryCode && !regionCountryCodes.includes(addressCountryCode)) {
    throw new MedusaError(
      MedusaError.Types.INVALID_DATA,
      "Cart address country does not belong to its region"
    )
  }
