...
describe("buildGLSFulfillmentOperationIdentity", () => {
  it("is stable when Medusa supplies fulfillment items in a different order", () => {
    const first = buildGLSFulfillmentOperationIdentity({
      environment: "testing",
      orderId: "order_1",
      attributes,
      items: [
    ...
    const second = buildGLSFulfillmentOperationIdentity({
      environment: "testing",
      orderId: "order_1",
      attributes,
      items: [
  ...
  it("changes across orders, environments, items, and quantities", () => {
    const baseline = buildGLSFulfillmentOperationIdentity({
      environment: "testing",
      orderId: "order_1",
      attributes,
    ...
    const alternatives = [
      ...
      buildGLSFulfillmentOperationIdentity({
        ...
        orderId: "order_1",
        attributes,
        items: [{ line_item_id: "item_2", quantity: 1 }],
      }),
      buildGLSFulfillmentOperationIdentity({
        environment: "testing",
        orderId: "order_1",
        attributes,
        items: [{ line_item_id: "item_1", quantity: 2 }],
      }),
      buildGLSFulfillmentOperationIdentity({
        environment: "testing",
        orderId: "order_1",
        attributes: { ...attributes, addressId: "parcelshop-two" },
        items: [{ line_item_id: "item_1", quantity: 1 }],
      }),
    ]

    expect(
      new Set(alternatives.map((alternative) => alternative.operationKey)).size
    ).toBe(alternatives.length)
    expect(
      alternatives.every(
        (alternative) => alternative.operationKey !== baseline.operationKey
      )
    ).toBe(true)
  })
  ...
  it("does not change when recalculated carrier attributes change during recovery", () => {
    const baseline = buildGLSFulfillmentOperationIdentity({
      environment: "testing",
      orderId: "order_1",
      attributes: { ...attributes, weight: 1, content: "Original" },
      items: [{ line_item_id: "item_1", quantity: 1 }],
    })
    const recalculated = buildGLSFulfillmentOperationIdentity({
      environment: "testing",
      orderId: "order_1",
      attributes: { ...attributes, weight: 2, content: "Updated" },
      items: [{ line_item_id: "item_1", quantity: 1 }],
    })
  ...
  it("rejects incomplete fulfillment items", () => {
    ...
    ).toThrow("line_item_id and quantity")
  })
    ...
    const query = {
      graph: vi.fn().mockResolvedValue({
        data: [
          {
            id: "order_1",
            fulfillments: [{ id: "ful_1" }, { id: "ful_2" }, { id: "ful_1" }],
          },
        ],
      }),
    }

    await expect(resolveOrderFulfillmentIds(query, "order_1")).resolves.toEqual(
      ["ful_1", "ful_2"]
    )
  })
})
