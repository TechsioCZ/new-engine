...
describe("resolveGLSCartContext", () => {
  it("derives the carrier country and Sales Channel from the cart", async () => {
    ...
    await expect(resolveGLSCartContext(query, "cart_1")).resolves.toEqual({
      countryCode: "SK",
  ...
  it("rejects an address outside the cart Region", async () => {
    ...
    await expect(resolveGLSCartContext(query, "cart_1")).rejects.toThrow(
      "does not belong to its region"
    )
  })
  ...
  it("rejects GLS account countries that are not supported storefront markets", async () => {
    const query = {
      graph: vi.fn().mockResolvedValue({
        data: [
          {
            id: "cart_1",
            sales_channel_id: "sc_hr",
            shipping_address: { country_code: "hr" },
            region: { countries: [{ iso_2: "hr" }] },
          },
        ],
      }),
    }
    ...
    await expect(resolveGLSCartContext(query, "cart_1")).rejects.toThrow(
      "unavailable for this cart country"
    )
  })
})
