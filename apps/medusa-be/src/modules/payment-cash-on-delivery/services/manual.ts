...
import { AbstractPaymentProvider, ModuleProvider, Modules, PaymentActions } from '@medusajs/framework/utils'
...
export class CashOnDeliveryPaymentProvider extends AbstractPaymentProvider<CashOnDeliveryPaymentProviderOptions> {
	...
	constructor(container: Record<string, unknown>, options: CashOnDeliveryPaymentProviderOptions = {}) {
	...
	async getPaymentStatus(input: GetPaymentStatusInput): Promise<GetPaymentStatusOutput> {
		return { status: input.data?.[CASH_ON_DELIVERY_DATA_KEY] === true ? 'authorized' : 'pending' }
	}
...
export default ModuleProvider(Modules.PAYMENT, { services: [CashOnDeliveryPaymentProvider] })
