...
export class Migration20260518143000 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table if not exists "qr_payment_config" ("id" text not null, "iban" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "qr_payment_config_pkey" primary key ("id"));`
    )
    this.addSql(
