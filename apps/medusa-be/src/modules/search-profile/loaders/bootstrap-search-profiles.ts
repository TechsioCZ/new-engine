...
import { readSearchProfiles, type SearchProfile } from '../../meilisearch/profiles'

type InternalSearchProfileService = {
	create: (data: Record<string, unknown>) => Promise<unknown>
	listAndCount: (filter: Record<string, unknown>) => Promise<[unknown[], number]>
}
...
const toBootstrapRecord = (profile: SearchProfile) => {
	...
	return {
		...
		minimum_ranking_score: profile.minimumRankingScore === automaticRankingScore ? null : profile.minimumRankingScore,
...
export default async function bootstrapSearchProfiles({ container }: LoaderOptions) {
	const logger = container.resolve(ContainerRegistrationKeys.LOGGER)
	const service = container.resolve<InternalSearchProfileService>('searchProfileService')
	...
	if (!process.env.MEILISEARCH_SEARCH_PROFILES?.trim()) {
		logger.info('Meilisearch: no search profiles configured; create one in Medusa Admin')
	...
	logger.info('Meilisearch: bootstrapped ' + profiles.length + ' search profile(s) into Medusa')
}
