...
const SearchProfile = model
	...
	.checks([
		...
		},
		{
			...
			expression: (columns) => columns.minimum_ranking_score + ' is null or (' + columns.minimum_ranking_score + ' >= 0 and ' + columns.minimum_ranking_score + ' <= 1)'
		},
		{
			name: 'search_profile_sync_status_check',
			expression: (columns) => columns.last_sync_status + ' in (\'never\', \'running\', \'succeeded\', \'failed\')'
		},
		{
			name: 'search_profile_sync_mode_check',
			expression: (columns) => columns.last_sync_mode + ' is null or ' + columns.last_sync_mode + ' in (\'normal\', \'full\')'
		}
	])
