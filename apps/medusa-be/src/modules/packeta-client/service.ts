...
const getConfigCacheKey = (configId: string) =>
  ["packeta", "config", configId].join(":")
const getBranchesCacheKey = (configId: string) =>
  ["packeta", "branches", configId].join(":")
...
export class PacketaClientModuleService extends MedusaService({
  ...
  constructor(
    ...
    this.logger_.info(
      "Packeta: Module service initialized with Admin-managed profiles"
    )
  }
  ...
  async listConfigProfiles(): Promise<PacketaConfigDTO[]> {
    ...
    return configs.map((config) =>
      this.decryptConfig(config as unknown as PacketaConfigDTO)
    )
  }

  async getConfig(
    environment?: PacketaEnvironment
  ): Promise<PacketaConfigDTO | null> {
    if (!environment) {
      return this.getActiveConfig()
    }

    const configs = await this.listPacketaConfigs({ environment }, { take: 1 })
  ...
  async updateConfig(
    ...
    if (filteredData.widget_countries) {
      filteredData.widget_countries = Array.from(
        new Set(filteredData.widget_countries)
      )
    }

    const isEnabled = filteredData.is_enabled ?? existing?.is_enabled ?? false
    const apiPassword =
      filteredData.api_password === undefined
        ? existing?.api_password
        : filteredData.api_password
    const widgetApiKey =
      filteredData.widget_api_key === undefined
        ? existing?.widget_api_key
        : filteredData.widget_api_key
    const widgetCountries =
      filteredData.widget_countries ?? existing?.widget_countries ?? []
    if (
      isEnabled &&
      (!(apiPassword && widgetApiKey) || widgetCountries.length === 0)
    ) {
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        "Packeta requires an API password, Widget API key, and at least one widget market before it can be enabled"
      )
    }
    ...
    return this.lockingService_.execute("packeta:activate-config", async () => {
      ...
      if (!target) {
        throw new MedusaError(
          ...
          `Packeta ${environment} profile was not found`
        )
      }
      ...
      if (environment === "production") {
        if (!confirmed) {
          throw new MedusaError(
            ...
            "Activating the Packeta production profile requires explicit confirmation"
          )
        }
        if (
          !(
            target.is_enabled &&
            target.api_password &&
            target.widget_api_key
          ) ||
          target.widget_countries.length === 0
        ) {
          throw new MedusaError(
            MedusaError.Types.INVALID_DATA,
            "Enable the Packeta production profile and complete its API and storefront widget settings before activating it"
          )
        }
      }
      ...
      const updatedTarget = updatedProfiles.find(
        (profile) => profile.id === target.id
      )
      if (!updatedTarget) {
        throw new MedusaError(
          MedusaError.Types.UNEXPECTED_STATE,
          "Packeta active profile was not persisted"
        )
      }
  ...
  private async resolveConfig(
    ...
    if (reference.config_id) {
      ...
      const decrypted = this.decryptConfig(
        config as unknown as PacketaConfigDTO
      )
      if (
        ...
        throw new MedusaError(
          MedusaError.Types.INVALID_DATA,
          "Packeta configuration reference does not match its profile"
        )
      }
      return decrypted
    }
  ...
  private async getCachedConfig(
    configId: string
  ): Promise<PacketaOptions | null | undefined> {
    if (!this.cacheService_) {
      return
    }
