...
const readRequiredString = (
  ...
  if (typeof value !== "string" || !value.trim()) {
    throw new SearchProfileConfigurationError(
      `MEILISEARCH_SEARCH_PROFILES[${profileIndex}].${field} must be a non-empty string`
    )
  }
...
const readBoolean = (
  ...
  if (typeof value !== "boolean") {
    throw new SearchProfileConfigurationError(
      `MEILISEARCH_SEARCH_PROFILES[${profileIndex}].${field} must be a boolean`
    )
  }
...
const readLimitedInteger = (options: {
  ...
  if (
    ...
    throw new SearchProfileConfigurationError(
      `MEILISEARCH_SEARCH_PROFILES[${options.profileIndex}].${options.field} must be an integer between 1 and ${options.maximum}`
    )
  }
...
const readRankingScore = (
  ...
  if (
    ...
    throw new SearchProfileConfigurationError(
      `MEILISEARCH_SEARCH_PROFILES[${profileIndex}].minimumRankingScore must be between 0 and 1`
    )
  }
...
const readAvailability = (
  ...
  throw new SearchProfileConfigurationError(
    `MEILISEARCH_SEARCH_PROFILES[${profileIndex}].availability must be "all" or "in-stock"`
  )
}
...
const readStringArray = (
  ...
  if (!Array.isArray(value)) {
    throw new SearchProfileConfigurationError(
      `MEILISEARCH_SEARCH_PROFILES[${profileIndex}].${field} must be an array`
    )
  }
    ...
    new Set(
      value.map((entry) => {
        if (typeof entry !== "string" || !entry.trim()) {
          throw new SearchProfileConfigurationError(
            `MEILISEARCH_SEARCH_PROFILES[${profileIndex}].${field} must contain only non-empty strings`
          )
        }
...
const readLimits = (
  ...
  const rawAutocomplete =
    raw.autocomplete &&
    typeof raw.autocomplete === "object" &&
    !Array.isArray(raw.autocomplete)
      ? (raw.autocomplete as Record<string, unknown>)
      : {}
...
const parseProfile = (
  rawProfile: RawSearchProfile,
  profileIndex: number
): SearchProfile => {
  const shop = normalizeSegment(
    readRequiredString(rawProfile.shop, "shop", profileIndex),
    "shop"
  )
  const domain = normalizeSegment(
    readRequiredString(rawProfile.domain, "domain", profileIndex),
    "domain"
  )
  const locale = normalizeSegment(
    readRequiredString(rawProfile.locale, "locale", profileIndex),
    "locale"
  )
  const strict = readBoolean(rawProfile.strict, false, "strict", profileIndex)
  const key = normalizeSegment(
    typeof rawProfile.key === "string" && rawProfile.key.trim()
      ? rawProfile.key
      : `${shop}-${domain}-${locale}`,
    "key"
  )
  ...
  return {
    ...
    minimumRankingScore: readRankingScore(
      rawProfile.minimumRankingScore,
      strict,
      profileIndex
    ),
    salesChannelIds: readStringArray(
      rawProfile.salesChannelIds,
      "salesChannelIds",
      profileIndex
    ),
    separateVariantResults: readBoolean(
      rawProfile.separateVariantResults,
      false,
...
const parseProfilesJson = (value: string): unknown[] => {
  ...
  try {
    ...
    throw new SearchProfileConfigurationError(
      `MEILISEARCH_SEARCH_PROFILES must be valid JSON: ${error instanceof Error ? error.message : String(error)}`
    )
  }

  if (!Array.isArray(parsed) || parsed.length === 0) {
    throw new SearchProfileConfigurationError(
      "MEILISEARCH_SEARCH_PROFILES must be a non-empty JSON array"
    )
  }
...
const parseProfileEntry = (entry: unknown, index: number): SearchProfile => {
  if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
    throw new SearchProfileConfigurationError(
      `MEILISEARCH_SEARCH_PROFILES[${index}] must be an object`
    )
  }
...
const assertUniqueProfileValues = (
  ...
  for (const profile of profiles) {
    ...
    if (seen.has(value)) {
      throw new SearchProfileConfigurationError(
        `Duplicate Meilisearch ${label}: ${value}`
      )
    }
...
export const loadSearchProfiles = async (
  ...
  const persisted =
    typeof service.listRuntimeProfiles === "function"
...
const hasSalesChannelAccess = (
  ...
  salesChannelIds: string[]
): boolean => profile.salesChannelIds.some((id) => salesChannelIds.includes(id))

const sortProfilesByKey = (profiles: SearchProfile[]): SearchProfile[] =>
  [...profiles].sort((left, right) => left.key.localeCompare(right.key))
...
export const resolveSearchProfile = (
  ...
  const requestedLocale = selection.locale
    ? normalizeLocale(selection.locale)
    : undefined
  const accessible = profiles.filter((profile) =>
    hasSalesChannelAccess(profile, salesChannelIds)
  )
  const localized = requestedLocale
    ? accessible.filter(
        (profile) => normalizeLocale(profile.locale) === requestedLocale
      )
    : accessible
