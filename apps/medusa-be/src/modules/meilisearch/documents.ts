...
export const cleanSearchText = (value: string): string =>
  value.replaceAll(TRADEMARK_SYMBOLS_REGEX, " ").replaceAll(/\s+/g, " ").trim()
...
export const cleanSearchDocument = <DocumentType extends UnknownRecord>(
  document: DocumentType
): DocumentType => (cleanValue(document) ?? {}) as DocumentType
...
export const buildProductSearchDocuments = (
  ...
  return [
    ...
    ...variants
      .filter((variant) => Boolean(getSearchDocumentId(variant.id)))
      .map((variant) =>
...
export const buildCategorySearchDocument = (
  ...
  cleanSearchDocument({
    ...
    parent_category_id:
      document.parent_category_id ?? asRecord(document.parent_category)?.id,
  })
