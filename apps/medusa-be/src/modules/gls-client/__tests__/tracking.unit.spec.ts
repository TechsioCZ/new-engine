...
describe('fetchPendingFulfillments', () => {
	it('continues beyond a full page of terminal records instead of starving live shipments', async () => {
		const failedPage = Array.from({ length: 100 }, (_, index) => fulfillment('failed_'.concat(String(index)), true))
		const graph = vi.fn().mockResolvedValueOnce({ data: failedPage }).mockResolvedValueOnce({ data: [fulfillment('live_1', false)] })
...
function fulfillment(id: string, deliveryFailed: boolean) {
	return {
		...
		delivered_at: null,
		data: { status: 'completed', packet_id: id, barcode: 'barcode_'.concat(id), supports_cod: false, config_id: 'config_testing', environment: 'testing', delivery_failed: deliveryFailed }
	}
}
