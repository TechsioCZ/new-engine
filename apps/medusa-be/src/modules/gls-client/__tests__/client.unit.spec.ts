...
describe("GLSClient", () => {
  it("requests and returns only active delivery points for the requested country", async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        ErrorCode: 0,
        Data: Array.from(gzipSync(JSON.stringify(points))),
      })
    )
  ...
  it("recovers one exact parcel by stable client reference", async () => {
        ...
        PrintDataInfoList: [
          {
            ...
            ParcelId: 10,
            ParcelNumber: 100,
            ParcelNumberWithCheckdigit: 1001,
          },
          {
            ClientReference: "NE-ONE",
            ParcelId: 11,
            ParcelNumber: 110,
            ParcelNumberWithCheckdigit: 1101,
          },
        ],
      })
    )
    ...
    const packet = await new GLSClient(options).findPacketByClientReference(
      "NE-ONE",
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toEqual(
      expect.objectContaining({
        PrintDateFrom: expect.stringMatching(DOT_NET_DATE_REGEX),
        PrintDateTo: expect.stringMatching(DOT_NET_DATE_REGEX),
      })
    )
  })
  ...
  it("stops when reconciliation finds duplicate carrier references", async () => {
    ...
    await expect(
      new GLSClient(options).findPacketByClientReference(
        "NE-DUPLICATE",
        new Date()
      )
  ...
  it("does not retry the carrier parcel creation write", async () => {
    ...
    await expect(
      new GLSClient(options).createPacket({
        number: "NE-ONE",
        name: "A",
        surname: "B",
    ...
    ).rejects.toThrow("503 - temporary failure")
