...
const resolveCustomerContext = async (
  ...
  return customer
    ? {
        customer: { id: customer.id, groups: customer.groups ?? [] },
...
const resolveRegion = async (
  ...
  if (!region?.currency_code) {
    throw new MedusaError(
      ...
      "The requested market region was not found"
    )
  }
...
const resolveProduct = async (
  ...
  if (!product) {
    throw new MedusaError(
      MedusaError.Types.NOT_FOUND,
      "The requested product variant was not found in this market"
    )
  }
...
export async function GET(
  ...
  const promotionService = request.scope.resolve<IPromotionModuleService>(
    Modules.PROMOTION
  )
  ...
  const tiers = await resolveApplicableVolumeDiscountTiers(
    promotionService,
    data as VolumeDiscountPromotionRecord[],
    {
