import type { ComputeActionContext, ComputeActionItemLine, ComputeActions, IPromotionModuleService } from '@medusajs/framework/types'
...
type VolumeDiscountCandidate = Pick<VolumeDiscountTier, 'promotion_id' | 'minimum_quantity' | 'percentage'> & {
	code: string
}
...
const toFiniteNumbers = (rule: PromotionRuleRecord): number[] =>
	(rule.values ?? []).map((entry) => Number(entry.value)).filter(Number.isFinite)
...
export const resolveExactSalesChannelId = (value: unknown): string => {
	...
	const uniqueSalesChannelIds = Array.from(new Set(salesChannelIds.filter((id): id is string => typeof id === 'string').map((id) => id.trim()).filter(Boolean)))

	if (uniqueSalesChannelIds.length !== 1) {
		throw new MedusaError(MedusaError.Types.INVALID_DATA, 'An exact Sales Channel is required for volume discounts')
	}

	return uniqueSalesChannelIds[0] ?? ''
}

export const resolveMinimumQuantity = (rules: PromotionRuleRecord[]): number | null => {
	const minimumQuantities = rules.flatMap((rule) => {
		if (rule.attribute !== 'items.quantity' || !['eq', 'gt', 'gte'].includes(String(rule.operator))) {
			return []
		}

		return toFiniteNumbers(rule).map((value) => rule.operator === 'gt' ? value + 1 : value)
	})
	const minimumQuantity = Math.max(...minimumQuantities)

	return Number.isInteger(minimumQuantity) && minimumQuantity >= 2 ? minimumQuantity : null
}

export const toVolumeDiscountCandidate = (promotion: VolumeDiscountPromotionRecord): VolumeDiscountCandidate | null => {
	...
	return { code: promotion.code, promotion_id: promotion.id, minimum_quantity: minimumQuantity, percentage: percentageValue }
}

const resolveCandidateAdjustmentAmount = (actions: ComputeActions[], candidate: VolumeDiscountCandidate, itemId: string): number =>
	actions.reduce((total, action) => {
		if (action.action !== 'addItemAdjustment' || action.code !== candidate.code || action.item_id !== itemId) {
			return total
		}
...
const evaluateVolumeDiscountCandidate = async (
	...
	const actions = await promotionService.computeActions([candidate.code], {
		currency_code: context.currency_code,
		...(context.customer ? { customer: context.customer } : {}),
		...(context.email ? { email: context.email } : {}),
		...(context.region ? { region: context.region } : {}),
		...(context.shipping_address ? { shipping_address: context.shipping_address } : {}),
		...(context.sales_channel_id ? { sales_channel_id: context.sales_channel_id } : {}),
		items: [{ ...context.item, quantity: candidate.minimum_quantity, subtotal: lineSubtotal, original_total: lineSubtotal }]
	}, { prevent_auto_promotions: true })
	const adjustmentAmount = resolveCandidateAdjustmentAmount(actions, candidate, context.item.id)

	if (!Number.isFinite(adjustmentAmount) || adjustmentAmount <= 0 || adjustmentAmount >= lineSubtotal) {
		return null
	}
...
export const resolveApplicableVolumeDiscountTiers = async (
	...
	const candidates = promotions
		...
		.filter((candidate): candidate is VolumeDiscountCandidate => candidate !== null)
	const tiersByQuantity = new Map<number, VolumeDiscountTier>()

	for (const candidate of candidates) {
		const tier = await evaluateVolumeDiscountCandidate(promotionService, candidate, context)
	...
	return Array.from(tiersByQuantity.values()).sort((left, right) => left.minimum_quantity - right.minimum_quantity)
}
