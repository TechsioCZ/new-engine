...
export async function GET(
  ...
  response: MedusaResponse<PacketaWidgetConfigResponse>
) {
  response.setHeader("Cache-Control", "no-store")

  const packetaService = safeResolve<PacketaClientModuleService>(
    request.scope,
    PACKETA_CLIENT_MODULE
  )
  if (!packetaService) {
  ...
  const enabled =
    config.is_enabled && apiKey.length > 0 && configuredCountries.length > 0
