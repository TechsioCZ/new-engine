...
Producer rules evaluate against `items.producer_ids`, which is injected through Medusa promotion workflow hooks from the product-producer module link. The currently exported hooks cover cart promotion updates, draft-order adjustment computation, and order adjustment previews. `refreshDraftOrderAdjustmentsWorkflow` also defines the needed hook in Medusa source, but is not currently exported from `@medusajs/medusa/core-flows`; see the TODO in `src/workflows/hooks/promotion-producer-context.ts`.
...
Unit tests are located in `__tests__/utils.unit.spec.ts` covering utility functions, Medusa route-loading assumptions, and producer context enrichment.
