...
export async function POST(request: MedusaRequest<PostAdminOrderExpeditionFulfillmentSchemaType>, response: MedusaResponse): Promise<void> {
	...
	const preparedFulfillment = await prepareFulfillment(query, order, request.validatedBody.location_id)

	const { result: fulfillment } = await createOrderFulfillmentWorkflow(request.scope).run({
		input: {
...
async function fetchOrder(query: Query, orderId: string): Promise<FulfillmentOrder> {
...
async function prepareFulfillment(query: Query, order: FulfillmentOrder, locationId: string): Promise<PreparedFulfillment> {
	...
	return {
		items: fulfillableItems.map((item) => ({ id: item.id, quantity: getFulfillableQuantity(item) })),
		shippingOptionId: selectedShippingOption.id
	}
}

async function fetchShippingOptions(query: Query, shippingOptionIds: string[]): Promise<FulfillmentShippingOption[]> {
	const { data } = await query.graph({ entity: 'shipping_option', fields: SHIPPING_OPTION_FIELDS, filters: { id: { $in: shippingOptionIds } } })
...
function matchesShippingProfile(item: FulfillmentLineItem, shippingOption: FulfillmentShippingOption): boolean {
	const itemShippingProfileId = item.variant?.product?.shipping_profile?.id

	return Boolean(itemShippingProfileId && shippingOption.shipping_profile_id === itemShippingProfileId)
}

function isShippingOptionAtLocation(shippingOption: FulfillmentShippingOption, locationId: string): boolean {
	return shippingOption.service_zone?.fulfillment_set?.location?.id === locationId
}

function isShippingOptionCompatible(items: FulfillmentLineItem[], shippingOption: FulfillmentShippingOption): boolean {
	return items.every((item) => matchesShippingProfile(item, shippingOption))
}

function getCompatibleShippingOptions(shippingOptions: FulfillmentShippingOption[], items: FulfillmentLineItem[]): FulfillmentShippingOption[] {
	return shippingOptions.filter((shippingOption) => isShippingOptionCompatible(items, shippingOption))
}
...
function toSafeQuantity(value: unknown): number {
	...
	if (!Number.isFinite(quantity) || quantity < 0) {
		throw new MedusaError(MedusaError.Types.INVALID_DATA, 'Order contains an invalid fulfillment quantity')
	}
