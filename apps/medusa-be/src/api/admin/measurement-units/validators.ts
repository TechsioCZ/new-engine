...
export const AdminGetMeasurementUnitsSchema = z
  .object({
    ...
    status: z.enum(["active", "all", "deleted"]).optional(),
  })
  .strict()

export const AdminGetMeasurementUnitProductsSchema = z
  .object({
    limit: z.coerce.number().int().min(1).max(100).optional().default(20),
    offset: z.coerce.number().int().min(0).optional().default(0),
    order: z.string().optional(),
    order_by: z.string().optional(),
    q: z.string().trim().optional(),
    status: z.enum(["active", "all", "deleted"]).optional().default("active"),
  })
  .strict()
...
>
export type AdminGetMeasurementUnitProductsSchemaType = z.infer<
  typeof AdminGetMeasurementUnitProductsSchema
>
...
>
export type AdminSetProductVariantMeasurementSchemaType = z.infer<
  typeof AdminSetProductVariantMeasurementSchema
>
