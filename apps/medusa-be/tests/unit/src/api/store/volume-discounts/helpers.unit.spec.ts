...
describe("resolveExactSalesChannelId", () => {
  ...
  it("rejects missing or ambiguous market scope", () => {
    expect(() => resolveExactSalesChannelId(undefined)).toThrow(
      "An exact Sales Channel is required"
    )
    expect(() => resolveExactSalesChannelId(["sc_sk", "sc_cz"])).toThrow(
      "An exact Sales Channel is required"
    )
  })
})
    ...
    target_rules: [
      {
        ...
        operator: "gte",
        values: [{ value: options.minimumQuantity }],
      },
    ],
  },
})
...
describe("toVolumeDiscountCandidate", () => {
  it("accepts only percentage item promotions allocated per item", () => {
    const validPromotion = createPromotion({
      code: "VOLUME-3",
      id: "promo_3",
      minimumQuantity: 3,
      percentage: 7,
    ...
    expect(
      toVolumeDiscountCandidate({
        ...fixedPromotion,
        application_method: {
          ...fixedPromotion.application_method,
          type: "fixed",
        },
      })
    ).toBeNull()
...
describe("resolveApplicableVolumeDiscountTiers", () => {
  ...
  it("uses Medusa to evaluate the exact market and customer context", async () => {
    ...
    const tiers = await resolveApplicableVolumeDiscountTiers(
      promotionService,
      [promotion],
      createContext()
    )
    ...
    expect(computeActions).toHaveBeenCalledWith(
      ["VOLUME-3"],
