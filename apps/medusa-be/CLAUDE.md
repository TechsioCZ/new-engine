...
Modules can't access others directly. Use: **Links** (associate), **Query** (retrieve), **Workflows** (orchestrate). Module container ≠ framework container → Query/Link unavailable in service, use workflows.
...
export default async function initLoader({
  container,
  options,
}: LoaderOptions<MyOptions>) {
  try {
    await service.create({ field: value })
  } catch (error) {
    // handle race condition (multiple containers)
...
**Functional core:** Extract pure functions to separate files for testability without runtime deps. **Shadowing:** Don't name vars `config` in admin routes (shadows `export const config = defineRouteConfig()`).
...
const MyEntity = model
  ...
  .checks([
    {
      name: "title_length",
      expression: (cols) => `LENGTH(${cols.title}) <= 200`,
    },
  ])
...
defineLink(
  A.linkable.a,
  { linkable: B.linkable.b, deleteCascades: true },
  {
    database: { extraColumns: { sort_order: { type: "integer" } } },
  }
)
defineLink(
  { linkable: M.linkable.m, field: "external_id" },
  External.linkable.e,
  { readOnly: true }
)
...
const { data } = await query.graph({
  entity: "fulfillment",
  fields: ["id", "data"],
  filters: { provider_id: "my_shipping_default" },
})
...
export const myRouteMiddlewares: MiddlewareRoute[] = [
  {
    ...
    middlewares: [validateAndTransformBody(PostSchema)],
  },
]

// route.ts - use req.validatedBody (type-safe, validated by middleware)
export async function POST(
  req: MedusaRequest<PostSchemaType>,
  res: MedusaResponse
) {
...
export default defineMiddlewares({
  routes: [
    ...myRouteMiddlewares,
    {
      ...
      matcher: "/store/x",
      middlewares: [
        validateAndTransformQuery(Schema, { defaults: ["id"], isList: true }),
      ],
    },
    {
      methods: ["POST"],
...
**HTTP** - Write for: business logic routes, security-critical middleware, multi-step DB ops. Skip for: `query.graph()` pass-through, external API routes, seed scripts.
...
moduleIntegrationTestRunner<MyModuleService>({
  ...
  testSuite: ({ service }) => {
    it("creates", async () =>
      expect((await service.createMyEntities({ title: "Test" })).title).toBe(
        "Test"
      ))
  },
})
...
After sessions: _"[Backend] Anything to add/update in CLAUDE.MD?"_ After rejecting suggestions: Consider what to clarify for next time.
