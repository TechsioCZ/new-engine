...
function resolveWorkspacePath(inputPath, root = workspaceRoot) {
...
async function buildOwnedDataArtifacts(args, modules, options = {}) {
  const datasetInput = await createDatasetInput(args, modules);
  const timestampValue = options.now === undefined ? new Date() : options.now();
  const timestamp =
  ...
  const artifactWorkspaceRoot = options.workspaceRoot ?? workspaceRoot;
  const outDir = resolveWorkspacePath(args.artifactOutDir, artifactWorkspaceRoot);
  const tempDir =
    options.tempDir ?? fs.mkdtempSync(path.join(os.tmpdir(), 'smart-suggest-owned-artifacts-'));
  ...
  try {
    ...
    return {
      ...
      artifactOutDir: path.relative(artifactWorkspaceRoot, outDir),
      complete,
      ...
      manifestPath: path.relative(artifactWorkspaceRoot, manifestPath),
      officialSnapshot: datasetInput.officialSnapshot,
