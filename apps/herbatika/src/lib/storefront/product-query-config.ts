...
export const PRODUCT_DETAIL_FIELDS = `${PRODUCT_CARD_FIELDS},description,images.url,categories.id,categories.name,categories.handle,categories.parent_category_id,categories.rank,brand.id,brand.title,brand.handle,options.id,options.title,variants.id,variants.title,variants.sku,variants.ean,variants.options.value,variants.options.option_id,+variants.metadata,+variants.calculated_price.price_per_unit,+metadata.content_sections,+metadata.related_products,+metadata.alternative_products,+metadata.primary_category_id`
