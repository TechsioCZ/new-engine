  ...
  if (!PPL_WIDGET_API_KEY) {
    return (
      <StatusText showIcon size="sm" status="error">
        Chýba konfigurácia PPL widgetu. Doplňte
        NEXT_PUBLIC_PPL_WIDGET_API_KEY a reštartujte dev server.
      </StatusText>
    );
  }
...
function formatPplAddress(accessPoint: PplAccessPoint) {
  const address = accessPoint.address;
  const addressParts = [address?.street, address?.zipCode, address?.city].filter(
    Boolean,
  );
