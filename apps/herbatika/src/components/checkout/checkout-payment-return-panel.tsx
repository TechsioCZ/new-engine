...
export function CheckoutPaymentReturnPanel() {
  ...
  const retryRequestKey = normalizeSearchParam(searchParams.get("retry"));
  ...
  }, [cartId, completedOrderId, isCancelled, retryRequestKey, returnError]);
...
function PaymentReturnStatusCard({
  ...
  return (
    <section className="mx-auto flex max-w-2xl flex-col gap-300 rounded-sm border border-border-primary bg-surface p-400 sm:p-550">
      <h1 className="font-rubik text-xl font-semibold text-fg-primary">
        {title}
      </h1>
