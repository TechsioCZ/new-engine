...
export function AccountDeactivationConfirmation({ token }: AccountDeactivationConfirmationProps) {
	...
	const handleConfirmAccountDeactivation = async () => {
		...
		try {
			const result = await confirmAccountDeactivationMutation.mutateAsync({ token: normalizedToken })
			if (!result.deleted) {
			...
			setConfirmationError(resolveErrorMessage(error, 'Potvrdzovací odkaz je neplatný alebo jeho platnosť vypršala.'))
		}
	}
			...
			<AccountSurface className = 'mx-auto max-w-3xl space-y-400'>
				<header className = 'space-y-200'>
					...
					<p className = 'text-fg-secondary text-sm'>
						Po potvrdení sa už nebudete môcť prihlásiť. Vaše existujúce objednávky zostanú bezpečne uložené.
					</p>
				</header>
				...
				{isConfirmed ? (
					<div className = 'space-y-300'>
						<StatusText align = 'start' showIcon status = 'success'>
							Účet bol zrušený. Pri opätovnej registrácii s rovnakým e-mailom sa obnoví pôvodný účet aj história objednávok.
						</StatusText>
					...
					<div className = 'space-y-300'>
						...
						<div className = 'flex flex-col gap-200 sm:flex-row sm:items-center'>
							<Button
								disabled = { !normalizedToken || confirmAccountDeactivationMutation.isPending }
							...
							<LinkButton as = { NextLink } href = '/' size = 'md' theme = 'outlined' variant = 'secondary'>
								Ponechať účet
							</LinkButton>
						</div>
					</div>
