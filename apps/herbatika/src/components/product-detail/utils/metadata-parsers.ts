...
const asMetadataNumber = (value: unknown): number | null => {
...
const resolveWarehouseStock = (stockSource: unknown): ProductWarehouseStock[] => {
  const stock = asRecord(stockSource);
  const warehouses = Array.isArray(stock?.warehouses) ? stock.warehouses : [];

  return warehouses
    .map((warehouse) => {
      const warehouseRecord = asRecord(warehouse);
      if (!warehouseRecord) {
        return null;
      }

      const name = asString(warehouseRecord.name);
      const quantity =
        asMetadataNumber(warehouseRecord.value) ??
        asMetadataNumber(warehouseRecord.quantity) ??
        asMetadataNumber(warehouseRecord.amount);
      ...
      return {
        name,
        quantity,
        location: asString(warehouseRecord.location),
      };
    })
    .filter((warehouse): warehouse is ProductWarehouseStock => Boolean(warehouse));
};

const sumWarehouseStock = (warehouses: ProductWarehouseStock[]) => {
  if (warehouses.length === 0) {
    return null;
  }

  return warehouses.reduce((total, warehouse) => total + warehouse.quantity, 0);
};
...
export const resolveOfferState = (
  ...
  const variantWarehouseStock = resolveWarehouseStock(variantMetadata?.stock);
  const sourceWarehouseStock = resolveWarehouseStock(stock);
  const warehouseStock =
    variantWarehouseStock.length > 0 ? variantWarehouseStock : sourceWarehouseStock;
  const warehouseStockAmount = sumWarehouseStock(warehouseStock);
  const stockAmount = warehouseStockAmount ?? asMetadataNumber(stock?.amount);
