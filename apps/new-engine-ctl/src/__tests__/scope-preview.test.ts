...
describe("scope-preview", () => {
  ...
  const explicitPreviewRejectPattern =
    /Explicit services are not deployable on lane preview: medusa-db/u
  const cloneToPreviewRejectPattern = /clone_to_preview is false/u
  const workspaceBinPath = nodePath.join(process.cwd(), "node_modules", ".bin")
