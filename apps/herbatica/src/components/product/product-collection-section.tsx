...
import {
  HerbaticaProductGrid,
  type HerbaticaProductGridLayout,
} from "@/components/product/herbatica-product-grid"
...
export function ProductCollectionSection(props: ProductCollectionSectionProps) {
  ...
  const subtitleClassNames = [
    ...
    .filter(Boolean)
    .join(" ")
  const skeletonLayout: HerbaticaProductGridLayout = isCarousel
    ? "collection"
    : props.layout
  let productContent: ReactNode
  if (shouldShowSkeleton) {
    productContent = <HerbaticaProductGridSkeleton layout={skeletonLayout} />
