...
import { mapHerbaticaAddressFormStateFromMedusaAddress } from "@/lib/storefront/cart/address-adapter"
...
const createCheckoutToggleStorageKey = (cartId?: string | null) =>
  cartId ? `herbatica.checkout-details.${cartId}` : null
...
> & {
  ...
  const resolvedShippingAddressValues =
    mapHerbaticaAddressFormStateFromMedusaAddress(shippingAddress)
  const resolvedBillingAddressValues =
    mapHerbaticaAddressFormStateFromMedusaAddress(billingAddress)
