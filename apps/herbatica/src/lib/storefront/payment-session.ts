...
export const HERBATICA_PAYMENT_RETURN_PATH = "/checkout/platba-navrat"
...
export const buildHerbaticaPaymentReturnUrl = ({
  cartId,
  providerId,
  ...
  const url = new URL(HERBATICA_PAYMENT_RETURN_PATH, origin)
...
export const buildHerbaticaPaymentSessionData = ({
  cart,
  cartId,
  providerId,
}: MedusaPaymentSessionDataInput): Record<string, unknown> => {
  const email = resolveCartEmail(cart)
  const returnUrl = buildHerbaticaPaymentReturnUrl({ cartId, providerId })
  const cancelUrl = buildHerbaticaPaymentReturnUrl({
    cartId,
