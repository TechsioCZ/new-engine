# Herbatica App Guidelines

## Scope & Precedence
- This file applies to `apps/herbatica`.
- Root `AGENTS.md` still applies unless this file overrides it.
- If work starts from the repo root and touches Herbatica, read this file before editing `apps/herbatica`.
...
- Put Herbatica visual differences in `src/styles/tokens/**`; avoid JSX `className` overrides that duplicate UI-kit component props/tokens.
- `src/styles/tokens/index.css` owns Tailwind, UI-kit token imports, Herbatica semantic/layout/spacing/typography tokens, and component token overrides.
...
- Herbatica composes the shared data layer in `src/lib/storefront/storefront.ts` and adjacent `storefront-definition*` files.
...
- Keep app-local code thin: SDK instance, field defaults, localized text, toasts, analytics, form DTOs, address adapters, and Herbatica-specific read models can stay here.
...
- From `apps/herbatica`, `pnpm dev` runs the app on `http://localhost:3001`.
- From the repo root, prefer `pnpm -C apps/herbatica <script>` for app scripts.
...
- Run `pnpm -C apps/herbatica lint` before handoff to match the Nx/CI Biome check.
