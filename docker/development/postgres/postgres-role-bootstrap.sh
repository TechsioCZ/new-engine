...
: "${PAYLOAD_DB_NAME:=${MEDUSA_APP_DB_NAME}}"
...
require_identifier "PAYLOAD_DB_USER" "$PAYLOAD_DB_USER"
require_identifier "PAYLOAD_DB_NAME" "$PAYLOAD_DB_NAME"
require_identifier "PAYLOAD_DB_SCHEMA" "$PAYLOAD_DB_SCHEMA"

if [ "$PAYLOAD_DB_USER" = "$POSTGRES_USER" ] ||
  [ "$PAYLOAD_DB_USER" = "$MEDUSA_APP_DB_USER" ] ||
  [ "$PAYLOAD_DB_USER" = "$MEDUSA_DEV_DB_USER" ]; then
  log "Refusing to bootstrap payload onto an existing elevated/app role; configure a dedicated PAYLOAD_DB_USER"
  exit 1
fi

if [ "$PAYLOAD_DB_NAME" != "$MEDUSA_APP_DB_NAME" ]; then
  log "Refusing to bootstrap payload into a different database than MEDUSA_APP_DB_NAME"
  exit 1
fi

if [ "$PAYLOAD_DB_SCHEMA" = "$MEDUSA_APP_DB_SCHEMA" ]; then
  log "Refusing to bootstrap payload onto MEDUSA_APP_DB_SCHEMA; configure a dedicated PAYLOAD_DB_SCHEMA"
  exit 1
fi
...
psql --username "$POSTGRES_USER" --dbname postgres \
  ...
  -v payload_db="$PAYLOAD_DB_NAME" \
  -v payload_schema="$PAYLOAD_DB_SCHEMA" \
  -v payload_user="$PAYLOAD_DB_USER" \
  -v payload_pass="$PAYLOAD_DB_PASSWORD" \
...
SELECT set_config('zane.payload_db', :'payload_db', false);
SELECT set_config('zane.payload_schema', :'payload_schema', false);
SELECT set_config('zane.payload_user', :'payload_user', false);
SELECT set_config('zane.payload_pass', :'payload_pass', false);
...
DO $do$
DECLARE
  payload_user text := current_setting('zane.payload_user');
  payload_pass text := current_setting('zane.payload_pass');
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = payload_user) THEN
    EXECUTE format('CREATE ROLE %I LOGIN', payload_user);
  END IF;
...
DECLARE
  ...
  app_user text := current_setting('zane.app_user');
  dev_user text := current_setting('zane.dev_user');
  payload_db text := current_setting('zane.payload_db');
  payload_schema text := current_setting('zane.payload_schema');
  payload_user text := current_setting('zane.payload_user');
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('REVOKE ALL PRIVILEGES ON DATABASE %I FROM %I', db_record.datname, payload_user);
  END LOOP;
  ...
  IF EXISTS (SELECT 1 FROM pg_database WHERE datname = payload_db) THEN
    EXECUTE format('REVOKE ALL PRIVILEGES ON DATABASE %I FROM %I', payload_db, payload_user);
    EXECUTE format('REVOKE TEMPORARY ON DATABASE %I FROM %I', payload_db, payload_user);
    EXECUTE format('GRANT CONNECT, CREATE ON DATABASE %I TO %I', payload_db, payload_user);
    EXECUTE format(
      'ALTER ROLE %I IN DATABASE %I SET search_path = %I, pg_catalog',
      payload_user,
      payload_db,
...
DECLARE
  ...
  type_record RECORD;
BEGIN
  EXECUTE format('CREATE SCHEMA IF NOT EXISTS %I AUTHORIZATION %I', payload_schema, payload_user);
  ...
  FOR schema_record IN
    ...
    WHERE nspname NOT LIKE 'pg_%'
      AND nspname <> 'information_schema'
      AND nspname <> payload_schema
  LOOP
    EXECUTE format('REVOKE ALL ON SCHEMA %I FROM %I', schema_record.nspname, payload_user);
  END LOOP;

  EXECUTE format('ALTER SCHEMA %I OWNER TO %I', payload_schema, payload_user);
  EXECUTE format('GRANT USAGE, CREATE ON SCHEMA %I TO %I', payload_schema, payload_user);
  EXECUTE format('GRANT USAGE ON SCHEMA %I TO %I', payload_schema, dev_user);

  FOR rel_record IN
    SELECT c.relkind, c.relname
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = payload_schema
      AND c.relkind IN ('r', 'p', 'v', 'm', 'S', 'f')
  LOOP
    IF rel_record.relkind IN ('r', 'p', 'f') THEN
      EXECUTE format('ALTER TABLE %I.%I OWNER TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON TABLE %I.%I TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT SELECT ON TABLE %I.%I TO %I', payload_schema, rel_record.relname, dev_user);
    ELSIF rel_record.relkind = 'v' THEN
      EXECUTE format('ALTER VIEW %I.%I OWNER TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT SELECT ON TABLE %I.%I TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT SELECT ON TABLE %I.%I TO %I', payload_schema, rel_record.relname, dev_user);
    ELSIF rel_record.relkind = 'm' THEN
      EXECUTE format('ALTER MATERIALIZED VIEW %I.%I OWNER TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT SELECT ON TABLE %I.%I TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT SELECT ON TABLE %I.%I TO %I', payload_schema, rel_record.relname, dev_user);
    ELSIF rel_record.relkind = 'S' THEN
      EXECUTE format('ALTER SEQUENCE %I.%I OWNER TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT USAGE, SELECT, UPDATE ON SEQUENCE %I.%I TO %I', payload_schema, rel_record.relname, payload_user);
      EXECUTE format('GRANT SELECT ON SEQUENCE %I.%I TO %I', payload_schema, rel_record.relname, dev_user);
    END IF;
  END LOOP;
  ...
  FOR routine_record IN
    ...
    WHERE n.nspname = payload_schema
  LOOP
    EXECUTE format('ALTER ROUTINE %s OWNER TO %I', routine_record.identity, payload_user);
    EXECUTE format('GRANT EXECUTE ON ROUTINE %s TO %I', routine_record.identity, payload_user);
    EXECUTE format('GRANT EXECUTE ON ROUTINE %s TO %I', routine_record.identity, dev_user);
  END LOOP;
  ...
  FOR type_record IN
    ...
    WHERE n.nspname = payload_schema
      AND t.typtype IN ('d', 'e')
  LOOP
    EXECUTE format('ALTER TYPE %I.%I OWNER TO %I', payload_schema, type_record.typname, payload_user);
    EXECUTE format('GRANT USAGE ON TYPE %I.%I TO %I', payload_schema, type_record.typname, payload_user);
    EXECUTE format('GRANT USAGE ON TYPE %I.%I TO %I', payload_schema, type_record.typname, dev_user);
  END LOOP;

  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I REVOKE ALL ON TABLES FROM PUBLIC', payload_user, payload_schema);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON TABLES TO %I', payload_user, payload_schema, payload_user);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT SELECT ON TABLES TO %I', payload_user, payload_schema, dev_user);

  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I REVOKE ALL ON SEQUENCES FROM PUBLIC', payload_user, payload_schema);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO %I', payload_user, payload_schema, payload_user);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT SELECT ON SEQUENCES TO %I', payload_user, payload_schema, dev_user);

  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I REVOKE ALL ON FUNCTIONS FROM PUBLIC', payload_user, payload_schema);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT EXECUTE ON FUNCTIONS TO %I', payload_user, payload_schema, payload_user);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT EXECUTE ON FUNCTIONS TO %I', payload_user, payload_schema, dev_user);

  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I REVOKE ALL ON TYPES FROM PUBLIC', payload_user, payload_schema);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT USAGE ON TYPES TO %I', payload_user, payload_schema, payload_user);
  EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA %I GRANT USAGE ON TYPES TO %I', payload_user, payload_schema, dev_user);
END
$do$;
SQL
fi
